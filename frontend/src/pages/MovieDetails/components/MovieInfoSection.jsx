...
export const MovieInfoSection = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading1(true);
            try {
                const movieDetailResponse = await axios.post(`${import.meta.env.VITE_API_URL}/movieDetail`, {
                    movieDetailsId,
                });

                const formattedRelDate = new Date(movieDetailResponse.data[0].release_date).toLocaleDateString("en-GB");
                const durationDetail1 = movieDetailResponse.data[0].duration.replace("h", " цаг");
                const duration = durationDetail1.replace("m", " минут");
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                setLoading2(true);
                const response = await axios.post(`${import.meta.env.VITE_API_URL}/movieWiseShowtime`, {
                    movieDetailsId,
            ...
            return (
                <li key={`3d ${singleTime.movie_start_time}`}>
                    <button
                        ...
                        onClick={() => {
                            dispatch(resetCart());
                            isAuthenticated && signedPerson.person_type === "Customer" ? navigate("/purchase") : dispatch(showLoginModal());
                        }}>
    ...
    return (
        <div className="section-movie-info container">
                ...
                <>
                    <div className="movie-info-grid-container">
                        <div className="movie-info-img-container">
                            <img className="movie-info-img" src={movieData && movieData.image_path} alt="Movie Photo" />
                        </div>
                        ...
                        <div className="movie-info-attr-container">
                            ...
                            <div className="movie-info-small-container ">
                                <svg xmlns="http://www.w3.org/2000/svg" className="movie-info-icon" viewBox="0 0 512 512">
                    ...
                    <div className="movie-info-description-container">
                        <h3 className="movie-info-description-heading">Хураангүй</h3>
                        <p className="movie-info-description">{movieData && movieData.synopsis}</p>
                    </div>
                </>
            )}
            ...
            <h3 className="movie-info-screen-heading">Үзвэрийн цагууд</h3>
