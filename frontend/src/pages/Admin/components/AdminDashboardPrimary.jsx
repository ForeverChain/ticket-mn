...
export const AdminDashboardPrimary = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response1 = await axios.get(`${import.meta.env.VITE_API_URL}/totalTickets`);
            ...
            try {
                const response2 = await axios.get(`${import.meta.env.VITE_API_URL}/totalPayment`);
            ...
            try {
                const response3 = await axios.get(`${import.meta.env.VITE_API_URL}/totalCustomers`);
    ...
    return (
        <section className="section-admin-summary container">
            <h2 className="form-admin-heading dash-heading">Нийт</h2>
            <div className="admin-dashboard-primary">
                <div className="dashboard-pri-card">
                    {loading1 ? <HashLoader size={30} color="#eb3656" /> : <p className="admin-dashboard-val">{ticketData.length > 0 && ticketData[0].total_tickets}</p>}
                    <div className="admin-dashboard-category">
                        <p>Нийт тасалбар зарагдсан тоо</p>
                ...
                <div className="dashboard-pri-card">
                    {loading2 ? (
                        ...
                        <p className="admin-dashboard-val">
                            {paymentData.length > 0 && paymentData[0].total_amount.toLocaleString("en-US")}
                            MNT
                        </p>
                    )}
                    <div className="admin-dashboard-category">
                ...
                <div className="dashboard-pri-card">
                    {loading3 ? <HashLoader size={26} color="#eb3656" /> : <p className="admin-dashboard-val">{customerData.length > 0 && customerData[0].total_customers}</p>}
                    <div className="admin-dashboard-category">
                        <p>Нийт үйлчлүүлэгч</p>
                        <svg xmlns="http://www.w3.org/2000/svg" className="admin-icon" viewBox="0 0 512 512">
