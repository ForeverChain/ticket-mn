...
export const AdminShownInModifySection = ({ selectedDate }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${import.meta.env.VITE_API_URL}/adminLatestShowDates`);
                const tempDateList = response.data.map((dateObj) => dateFormatter(dateObj.showtime_date));
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (selectedShowDate !== "") {
                try {
                    const response = await axios.post(`${import.meta.env.VITE_API_URL}/adminShowtimes`, {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.post(`${import.meta.env.VITE_API_URL}/movieReplaceFrom`, {
                    email: signedPerson.email,
                    password: signedPerson.password,
                    selectedShowtime,
                });
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.post(`${import.meta.env.VITE_API_URL}/movieReplaceTo`, {
                    email: signedPerson.email,
                    password: signedPerson.password,
                    selectedShowtime,
                });
        ...
        fetchData();
    }, [selectedReplace, selectedShowtime, signedPerson.email, signedPerson.password]);
    ...
    const handleMovieSwap = async (e) => {
        ...
        try {
            setLoading(true);
            const res = await axios.post(`${import.meta.env.VITE_API_URL}/movieSwap`, {
                email: signedPerson.email,
                password: signedPerson.password,
                selectedAlt,
                selectedShowtime,
    ...
    const latestShowDatesHtml = latestShowDates.map((dateObj, i) => (
        <div className="admin-radio-input-container" key={i + 1} style={checkedColor(dateObj.formattedDate, selectedShowDate)}>
            <input type="radio" id={i + 1} name="Select Showdate" value={dateObj.formattedDate} onChange={(e) => handleSelectedDate(e)} checked={dateObj.formattedDate === selectedShowDate} />

            <label className="form-admin-input-detail" htmlFor={dateObj.formattedDate}>
                {dateObj.visualDate}
            </label>
        </div>
    ));
    ...
    const latestShowtimesHtml = showtimeData.map((showtimeObj, i) => (
        <div className="admin-radio-input-container" key={i + 1} style={checkedColor(showtimeObj.id, selectedShowtime)}>
            <input type="radio" id={i + 1} name="Select Showtime" value={showtimeObj.id} onChange={(e) => handleSelectedShowtime(e)} checked={showtimeObj.id === selectedShowtime} />
    ...
    const replaceOptionsHtml = movieReplaceData?.map((movieObj, i) => (
        <div className="admin-radio-input-container" key={i + 1} style={checkedColor(movieObj.movie_id, selectedReplace)}>
            <input type="radio" id={i + 1} name="Replace Movie" value={movieObj.movie_id} onChange={(e) => handleSelectedReplace(e)} checked={movieObj.movie_id === selectedReplace} />
    ...
    const newMovieOptionsHtml = movieAltData.map((movieObj, i) => (
        <div className="admin-radio-input-container" key={i + 1} style={checkedColor(movieObj.id, selectedAlt)}>
            <input type="radio" id={i + 1} name="Choose New Movie" value={movieObj.id} onChange={(e) => handleSelectedAlt(e)} checked={movieObj.id === selectedAlt} />
        ...
        <section className="section-admin-movie-modify">
            <div className="section-movie-playlist container">
                <div className="form-heading-container">
                    <h2 className="form-admin-heading playlist">Киноны тоглуулах жагсаалтыг шинэчлэх</h2>
                ...
                {moviePlaylistDropDown && (
                    <form className="form-admin-showtime-add">
                        <div className="form-admin-radio-options">{latestShowDatesHtml}</div>

                        {selectedShowDate.length > 0 && (
                            <>
                                <h3 className="admin-heading-secondary">Цагаа сонгох</h3>
                                <div className="form-admin-radio-options">{latestShowtimesHtml}</div>
                            </>
                        )}

                        {selectedShowtime !== "" && (
                            <>
                                <h3 className="admin-heading-secondary">Солих</h3>
                                <div className="form-admin-radio-options">{replaceOptionsHtml}</div>
                            </>
                        )}

                        {selectedReplace !== "" && (
                            <>
                                <h3 className="admin-heading-secondary">Солих киногоо сонго</h3>
                                <div className="form-admin-radio-options">{newMovieOptionsHtml}</div>
                            </>
                        )}
                        {selectedAlt !== "" && (
                            <button className="btn-admin" type="submit" disabled={loading} onClick={handleMovieSwap}>
                                Баталгаажуулах
                            </button>
                        )}
                    </form>
