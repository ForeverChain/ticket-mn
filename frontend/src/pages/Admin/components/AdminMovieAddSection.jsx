...
export const AdminMovieAddSection = () => {
    ...
    const handleMovieInfo = (e) => {
        ...
        const value = name === "genres" || name === "directors" ? e.target.value.split(",") : e.target.value;
    ...
    const movieAdd = async (e) => {
        ...
        if (movieInfo.movieName !== "" && movieInfo.imagePath !== "" && movieInfo.language !== "" && movieInfo.description !== "" && movieInfo.rating !== "" && movieInfo.duration !== "" && movieInfo.cast !== "" && movieInfo.relDate !== "" && movieInfo.genres !== "" && movieInfo.directors !== "") {
            try {
                // Add the movie
                setLoading(true);
    ...
    return (
        <section className="section-admin-movie-add container">
            <div className="form-heading-container">
                <h2 className="form-admin-heading">Кино нэмэх</h2>
            ...
            {adminMovieDropDown && (
                <form className="form-movie-add" onSubmit={movieAdd}>
                    <div>
                        <p>Киноны нэр:</p>
                        <input name="movieName" onChange={(e) => handleMovieInfo(e)} type="text" placeholder="Enter Movie Name" />
                    </div>

                    <div>
                        <p>Зурагын хаяг:</p>
                        <input name="imagePath" onChange={(e) => handleMovieInfo(e)} type="text" placeholder="Enter Image path" />
                    </div>

                    <div>
                        <p>Хэл:</p>
                        <input name="language" onChange={(e) => handleMovieInfo(e)} type="text" placeholder="Enter Movie Language" />
                    </div>

                    <div>
                        <p>Хураангүй:</p>
                        <input name="description" onChange={(e) => handleMovieInfo(e)} placeholder="Enter Movie's Brief Description" />
                    </div>

                    <div>
                        <p>Үнэлгээ:</p>
                        <input name="rating" onChange={(e) => handleMovieInfo(e)} type="text" placeholder="Enter Movie Rating" />
                    </div>

                    <div>
                        <p>Урт:</p>
                        <input name="duration" onChange={(e) => handleMovieInfo(e)} type="text" placeholder="Enter Movie Duration" />
                    </div>

                    <div>
                        <p>Гол жүжигчид:</p>
                        <input name="cast" onChange={(e) => handleMovieInfo(e)} type="text" placeholder="Enter Movie's Main Actor/Actress Name" />
                    </div>

                    <div>
                        <p>Гарсан огноо:</p>
                        <input name="relDate" onChange={(e) => handleMovieInfo(e)} type="text" placeholder="(yyyy-mm-dd) format" />
                    </div>

                    <div>
                        <p>Киноны төрөл:</p>
                        <input name="genres" onChange={(e) => handleMovieInfo(e)} type="text" placeholder="Enter separate Genres with comma" />
                    </div>

                    <div>
                        <p>Movie Directors:</p>
                        <input name="directors" onChange={(e) => handleMovieInfo(e)} type="text" placeholder="Enter separate Directors with comma" />
                    </div>
