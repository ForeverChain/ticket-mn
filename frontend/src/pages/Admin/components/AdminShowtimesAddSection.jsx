...
export const AdminShowtimesAddSection = ({ selectedShowDate, setSelectedShowDate, handleSelectedDate }) => {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get(`${import.meta.env.VITE_API_URL}/lastShowDate`);
    ...
    for (let i = 0; i < 4; i++) {
        ...
        showDateHtml.push(
            <div className="admin-radio-input-container" key={i + 1} style={checkedColor(curDateStr)}>
                <input type="radio" id={i + 1} name="Select Showdate" value={curDateStr} onChange={(e) => handleSelectedDate(e)} checked={curDateStr === selectedShowDate} />
    ...
    const showtimeAdd = async (e) => {
        ...
        try {
            setLoading(true);
            const response1 = await axios.post(`${import.meta.env.VITE_API_URL}/showdateAdd`, {
    ...
    return (
        <section className="section-admin-showtimes container">
            <div className="form-heading-container">
                <h2 className="form-admin-heading">Үзвэрийн цаг нэмэх</h2>
                <button className="btn-admin-arrow" onClick={toggleAdminShowtimesSection}>
            ...
            {adminShowtimeDropdown && (
                <form className="form-admin-showtime-add">
                    ...
                    <button className="btn-admin" type="submit" disabled={loading} onClick={showtimeAdd}>
                        {loading ? "Loading..." : "Баталгаажуулах"}
                    </button>
                </form>
            )}
        </section>
