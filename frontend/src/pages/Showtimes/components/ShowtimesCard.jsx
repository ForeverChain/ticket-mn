...
export const ShowtimesCard = (props) => {
    ...
    const show_types_3d =
        ...
        dates3d.map((curDate, id) => {
            ...
            return (
                <div className="showtimes-schedule" key={`${props["movie_name"]} 3d ${id}`}>
    ...
    const show_types_2d =
        ...
        dates2d.map((curDate, id) => {
            ...
            return (
                <div key={`${props["movie_name"]} 2d ${id}`} className="showtimes-schedule">
                    <h3 className="showtimes-date">{formattedDate}</h3>
                    <ul className="showtimes-startime-btn-list">{curStartTimes}</ul>
                </div>
            );
        ...
        <div className="showtimes-card">
            <div className="showtimes-card-leftpart">
                <div className="showtimes-img-box">
                    <img className="showtimes-img" src={props.image_path} alt={props.movie_name} />
                </div>

                <h2 className="showtimes-title">{props.movie_name}</h2>
                <button className="showtimes-details-btn" onClick={() => navigate(`/movieDetails/${props.id}`)}>
                    Дэлгэрэнгүй харах
                </button>
            </div>
            ...
            <div className="showtimes-screen-container">
                {dates3d.length !== 0 && (
                    <div className="showtimes-schedule-container-3d" style={dates2d.length !== 0 ? { borderBottom: "1px solid #313441" } : {}}>
