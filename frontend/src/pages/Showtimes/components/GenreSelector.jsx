...
export const GenreSelector = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setGenreData([{ genre: "Бүх" }, ...response.data]);
                setLoading(false);
            } catch (err) {
                console.log(err);
            }
    ...
    const genreOptions = genreData?.map(({ genre }, idx) => {
        return (
            <div className="genre-input-container" key={idx} style={checkedColor(genre)}>
                <input
                    type="radio"
                    id={idx}
                    name="Select Genre"
    ...
    return loading ? (
        ...
        <div className="genre-container">
            <div className="genre-icon-container">
                <svg xmlns="http://www.w3.org/2000/svg" className="genre-icon" viewBox="0 0 512 512">
                    <path
