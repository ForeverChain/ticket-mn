...
export const DateSelector = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                const response = await axios.post(`${import.meta.env.VITE_API_URL}/showtimesDates`, {
                    theatreId,
                });
    ...
    const dateOptions = Array.isArray(showDatesData)
        ? showDatesData.map((dateData, idx) => {
              ...
              const monthNumber = new Date(dateData.showtime_date).toLocaleString("en-us", {
                  month: "numeric",
              });
              ...
              return (
                  <div className="date-input-container" key={idx} style={checkedColor(formattedDate)}>
                      <input type="radio" id={idx} name="Select Date" value={formattedDate} onChange={(e) => dispatch(setShowDate(e.target.value))} checked={formattedDate === userDate} />
    ...
    return (
        <div>
            <form>
                <div className="form-item-heading">Өдөр сонгох</div>
                {!loading ? <div className="form-item-options">{dateOptions}</div> : <HashLoader color="#eb3656" />}
            </form>
        </div>
    );
};
