...
export const PurchaseSection = () => {
    ...
    const { id: theatreId, location: theatreLocation } = useSelector((store) => store.currentLocation);
    const { payment_method: userPayMethod, showtime_date: userDate, movie_id: userMovieId, showtime_id: userShowtimeId, seat_id_list: userSeatList, hall_id: userHallId, seat_price: userSeatPrice } = useSelector((store) => store.cart);
    ...
    const currentMovie = userMovieId !== "" && movieData.find((movieObj) => movieObj.id === userMovieId);

    const curHallObj = userHallId && hallData.find((hall) => hall.hall_id === userHallId && hall.showtime_id === userShowtimeId);

    const userSeats = seatsData && seatsData.filter((seatData) => userSeatList.includes(seatData.seat_id));
    ...
    const handleTicketPurchase = async () => {
        try {
            ...
            const paymentResponse = await axios.post(`${import.meta.env.VITE_API_URL}/payment`, {
            ...
            const recentPurchaseResponse = await axios.post(`${import.meta.env.VITE_API_URL}/recentPurchase`, {
                paymentID,
            });
        ...
        <section className="section-purchase">
            <div className="purchase-container container">
                <div className="purchase-section-left">
                    ...
                    {userDate !== "" && <MovieSelector movieData={movieData} setMovieData={setMovieData} />}

                    {userMovieId !== "" && <PictureQualitySelector hallData={hallData} setHallData={setHallData} />}

                    {userShowtimeId !== "" && <SeatSelector seatsData={seatsData} setSeatsData={setSeatsData} />}

                    {userSeatList.length > 0 && <PayMethodSelector />}
                </div>

                <div className="purchase-section-right">
                    <h2 className="ticket-container-heading">Тасалбарын мэдээлэл</h2>

                    <div className="ticket-container">
                        {currentMovie && (
                            <div className="ticket-heading">
                                <div className="ticket-movie-img-cont">
                                    <img className="ticket-movie-img" src={currentMovie.image_path} alt="selected movie image" />
                                </div>

                                {currentMovie && (
                                    <div className="ticket-primary-info">
                                        {curHallObj && <p className="ticket-movie-screen">{curHallObj.show_type}</p>}
                                        <p className="ticket-movie-name">{currentMovie.movie_name}</p>
                            ...
                            <ul className="ticket-info-list">
                                <li className="ticket-info-item">
                                    <div className="ticket-info-category">
                                        <svg xmlns="http://www.w3.org/2000/svg" className="ticket-icon" viewBox="0 0 512 512">
                                    ...
                                    <p className="ticket-info-val">{theatreLocation !== "" ? theatreLocation : "--"}</p>
                                </li>
                                ...
                                <li className="ticket-info-item">
                                    ...
                                    <p className="ticket-info-val">{formattedDate ? formattedDate : "--"}</p>
                                </li>
                                ...
                                <li className="ticket-info-item">
                                    ...
                                    <p className="ticket-info-val">{curHallObj ? curHallObj?.hall_name : "--"}</p>
                                </li>
                                ...
                                <li className="ticket-info-item">
                                    ...
                                    <p className="ticket-info-val">{userHallId ? curHallObj?.movie_start_time : "--"}</p>
                                </li>
                                ...
                                <li className="ticket-info-item">
                                    ...
                                    <p className="ticket-info-val">{userSeats ? userSeats.length : "--"}</p>
                                </li>
                                ...
                                <li className="ticket-info-item">
                                    ...
                                    <p className="ticket-info-val">{userSeats && userSeats.length !== 0 ? userSeats.map((seat) => seat.seat_name).join(", ") : "--"}</p>
                                </li>
                                ...
                                <li className="ticket-info-item">
                                    <div className="ticket-info-category">
                                        ...
                                        <p>Төлбөрийн нөхцөл</p>
                                    </div>

                                    <p className="ticket-info-val">{userPayMethod && userPayMethod.length > 0 ? userPayMethod : "--"}</p>
                                </li>
                                ...
                                <li className="ticket-info-item">
                                    ...
                                    <p className="ticket-info-val">{userSeatPrice && userSeats ? `BDT ${userSeatPrice * userSeats.length}TK` : "--"}</p>
                                </li>
                            </ul>
                        </div>

                        <button className={btnDisabled ? "ticket-btn-disabled" : "ticket-btn"} onClick={handleTicketPurchase} disabled={btnDisabled}>
                            {loading ? <BarLoader color="#e6e6e8" /> : "Тасалбар худалдаж авах"}
                        </button>
                    </div>
                </div>
            </div>
