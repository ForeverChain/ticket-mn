...
export const LocationSelector = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                const response = await axios.get(`${import.meta.env.VITE_API_URL}/theatres`);
    ...
    const handleLocationSelection = (e) => {
        const selectedLocationObj = locationData.find((locationObj) => locationObj.id === Number(e.target.value));
    ...
    return !loading ? (
        <div className="location-select-container ">
            <select id="location-selector" onChange={handleLocationSelection} value={userLocation?.id}>
                {locationOptions}
            </select>

            <p className="selected-location">
                Байршил: <span>{userLocation?.location}</span>
            </p>
            <p className="selected-theatre">
                Кино театр: <span>{userLocation?.name}</span>
            </p>
        </div>
    ) : (
