...
export const Footer = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${import.meta.env.VITE_API_URL}/locationDetails`);
        ...
        <section className="section-footer container">
            {pageName === "home" ? (
                <HashLink className="footer-logo-container" to="#headerTop">
                    <svg xmlns="http://www.w3.org/2000/svg" className="footer-logo-icon" viewBox="0 0 512 512">
                    ...
                    <h1 className="footer-logo-text">Uzwer</h1>
                </HashLink>
            ) : (
            ...
            <h3 className="footer-heading">Кино театрууд</h3>

            <p className="copyright">
                Copyright &copy; 2023 by NELOY SAHA, Inc. This work is licensed under the terms of the <a href="https://www.gnu.org/licenses/gpl-3.0.html">GNU General Public License, version 3 or later (GPL-3.0-or-later)</a>.
            </p>

            <div className="footer-address-container">{loading ? <HashLoader color="#eb3656" /> : locations}</div>
        </section>
    );
};
