...
app.get("/latestMovies", (req, res) => {
    const sql = "SELECT m.id, m.name, m.image_path, m.rating, m.duration, m.release_date as release_date, GROUP_CONCAT(g.genre SEPARATOR ', ') as genres FROM movie m INNER JOIN movie_genre g ON m.id = g.movie_id GROUP BY m.id ORDER BY release_date DESC LIMIT 6";
...
app.post("/uniqueMovies", (req, res) => {
    ...
    const sql = "SELECT DISTINCT M.id,M.duration, M.name AS movie_name, M.image_path FROM movie M JOIN shown_in SI ON M.id = SI.movie_id JOIN showtimes S ON SI.showtime_id = S.id JOIN hall H ON SI.hall_id = H.id WHERE H.theatre_id = ? AND S.showtime_date = ?";
...
app.post("/halls", (req, res) => {
    ...
    const sql = "SELECT H.id AS hall_id, H.name AS hall_name, SI.showtime_id, S.show_type,S.movie_start_time, S.price_per_seat FROM hall H JOIN shown_in SI ON H.id = SI.hall_id JOIN showtimes S ON SI.showtime_id = S.id WHERE H.theatre_id = ? AND S.showtime_date = ? AND SI.movie_id = ?";
...
app.post("/purchaseTicket", (req, res) => {
    ...
    const sql = "INSERT INTO ticket (price,purchase_date,payment_id,seat_id,hall_id,movie_id,showtimes_id) VALUES (?,?,?,?,?,?,?)";

    db.query(sql, [price, date, payment_id, seat_id, hall_id, movie_id, showtime_id], (err, data) => {
        if (err) return res.json(err);
...
app.post("/login", (req, res) => {
    ...
    db.query(sql, [email, password], (err, data) => {
        ...
        if (data.length === 0) {
            return res.status(404).json({ message: "Уучлаарай, Хэрэглэгч олдсонгүй!" });
        }
...
app.post("/otherMovies", (req, res) => {
    ...
    sql = "SELECT m.id, m.name, m.image_path, m.rating, m.duration, m.release_date as release_date, GROUP_CONCAT(g.genre SEPARATOR ', ') as genres FROM movie m INNER JOIN movie_genre g ON m.id = g.movie_id GROUP BY m.id HAVING m.id!=?";
...
app.post("/adminMovieAdd", (req, res) => {
  ...
  (?,?,?,?,?,?,?,?)`;
    ...
    db.query(sql0, [email, password, "Admin"], (err, data) => {
        ...
        db.query(sql1, [name, image_path, language, synopsis, rating, duration, top_cast, release_date], (err1, data1) => {
            if (err1) return res.json(err1);
