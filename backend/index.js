...
app.get("/latestMovies", (req, res) => {
  const searchWrd = req.query.search || "";

  const sql =
    "SELECT m.id, m.name, m.image_path, m.rating, m.duration, m.release_date as release_date, GROUP_CONCAT(g.genre SEPARATOR ', ') as genres FROM movie m INNER JOIN movie_genre g ON m.id = g.movie_id WHERE  m.name LIKE ? GROUP BY m.id ORDER BY release_date DESC LIMIT 6";
  const searchParam = `%${searchWrd}%`;

  db.query(sql, [searchParam], (err, data) => {
...
app.post("/showtimes", (req, res) => {
  ...
  const searchWrd = req.body.searchWrd;
  console.log(searchWrd);

  // Modify the SQL queries to include the search condition using the LIKE operator
  let sql1 = `SELECT M.id, M.name AS movie_name, M.image_path, S.showtime_date, S.movie_start_time, S.show_type, MG.genre 
              FROM theatre T 
              ...
              WHERE T.name = ? AND MG.genre = ? AND M.name LIKE ? 
              ORDER BY S.showtime_date ASC`;

  let sql2 = `SELECT M.id, M.name AS movie_name, M.image_path, S.showtime_date, S.movie_start_time, S.show_type, MG.genre 
              FROM theatre T 
              JOIN hall H ON T.id = H.theatre_id 
              JOIN shown_in SI ON H.id = SI.hall_id 
              JOIN showtimes S ON SI.showtime_id = S.id 
  ...
  // Include the '%' wildcard for partial matching
  const searchParam = `%${searchWrd}%`;

  // Choose the appropriate SQL query based on whether userGenre is "Бүх"
  const sqlQuery = userGenre === "Бүх" ? sql2 : sql1;

  // Execute the SQL query with parameters
  db.query(
    sqlQuery,
    userGenre === "Бүх"
      ? [theatreName, searchParam]
      : [theatreName, userGenre, searchParam],
    (err, data) => {
      if (err) {
  ...
  db.query(
    ...
    (err, data) => {
      if (err) {
        return res
          .status(500)
          .json({ message: "Уучлаарай, дахиад оролдон уу!" });
      }

      return res.status(200).json({ message: "Амжилттай бүртгүүллээ" });
    }
  );
});
...
app.post("/login", (req, res) => {
  ...
  db.query(sql, [email, password], (err, data) => {
    ...
    if (data.length === 0) {
      return res
        .status(404)
        .json({ message: "Уучлаарай, Хэрэглэгч олдсонгүй!" });
    }
